...
func (p *Parser) Tool() string { return "yarn" }

func (p *Parser) Detect(cmd string, args []string) bool {
	return strings.HasSuffix(cmd, "yarn")
}
...
func (p *Parser) Summary() domain.Summary {
	...
	if p.summary.Status == "failure" {
		p.summary.SummaryText = "\u2717 yarn command failed"
	} else {
		p.summary.SummaryText = "\u2713 yarn command successful"
	}
	return p.summary
}
